...
import torch
from torchvision.models import resnet18, resnet34, resnet50, resnet101, resnext101_32x8d, wide_resnet101_2
...
class DensenetFeatureExtractor:
    ...
    def __init__(self, name='densenet121'):
        super().__init__()

        networks = {"densenet169": densenet169}

        self.densenet = networks[name](pretrained=True)
        # del self.resnet.avgpool, self.resnet.fc
        for m in self.densenet.parameters():
            m.requires_grad = False
        self.densenet.eval()

        self._out_channels = odict(  # Deep-to-shallow order is required by SegNetwork
            layer4=get_out_channels(self.densenet.features))
