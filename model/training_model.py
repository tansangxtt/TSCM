...
class TrainerModel(nn.Module):
    ...
    def forward(self, images, labels, meta):
        ...
        for i in range(1, len(images)):
            ...
            # y_transformer = self.transformer.backbone(images[i].to(self.device))
            # self.transformer.decode_head.forward_train(y_transformer, meta[0],
            #                                          labels[i], "configs/SETR/SETR_PUP_768x768_40k_cityscapes_bs_8.py")
            # cLonglabel = labels[i].type(torch.cuda.LongTensor)
            # self.transformer.forward_train(images[i].to(self.device), meta[0],
            #                                            cLonglabel)

            #s = self._forward_with_transformer(images[i].to(self.device))
    ...
    def _forward_with_transformer(self, image):
        ...
        # process transformer here
        y_transformer = self.transformer.backbone(image)
        for i in y_transformer:
            print(i.shape)

        print(len(y_transformer))
