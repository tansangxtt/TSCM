...
class Parameters:
    ...
    def __init__(self, weights, fast=False, device="cuda:0"):
        ...
        if self.in_channels == 1024:
            ...
            self.feature_extractor = "densenet169"
        else:
            raise ValueError
    ...
    def get_model(self, model_name):
        ...
        if "dense" in model_name:
            refinement_layers_channels = {"layer4": 1664}
...
if __name__ == '__main__':
    ...
    paths = dict(
        ...
        davis=Path(__file__).parent / "dataset/DAVIS-2017-trainval-480p/DAVIS",  # DAVIS dataset root
        yt2018=Path(__file__).parent / "dataset/ytvos",  # YouTubeVOS 2018 root
        output=Path(__file__).parent / "output",  # Output path
    )
    ...
    tracker = p.get_model(args.model)
