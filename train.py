...
from model.feature_extractor import ResnetFeatureExtractor, DensenetFeatureExtractor
...
class ModelParameters:
    ...
    def get_model(self):
        ...
        if "densenet" in self.feature_extractor:
            extractor = DensenetFeatureExtractor(name=self.feature_extractor).to(self.device)
            self.refnet_params = edict(
                refinement_layers=["layer4"],
                nchannels=64, use_batch_norm=True
            )
            self.disc_params = edict(
                layer="layer4", in_channels=1664, c_channels=1024, out_channels=1,
                init_iters=(5, 10, 10, 10, 10), update_iters=(10,), update_filters=True,
                filter_reg=(1e-5, 1e-4), precond=(1e-5, 1e-4), precond_lr=0.1, CG_forgetting_rate=75,
                memory_size=20, train_skipping=8, learning_rate=0.1,
                pixel_weighting=None, device=self.device
        ...
        if "densenet" in self.feature_extractor:
            ft_channels = {"layer4": 1664}

        seg_network = SegNetwork(1, p.nchannels, ft_channels, p.use_batch_norm)
        path = "configs/SETR/SETR_PUP_768x768_40k_cityscapes_bs_8.py"
...
if __name__ == '__main__':
    ...
    # path = "configs/SETR/SETR_PUP_768x768_40k_cityscapes_bs_8.py"
    # model = get_attention_model(path)
    # print(model)

    paths = dict(
        dv2017=Path(__file__).parent / "dataset/DAVIS-2017-trainval-480p/DAVIS",
        ytvos2018=Path(__file__).parent / "dataset/ytvos",
        checkpoints=Path(__file__).parent / "workspace/checkpoints",  # Checkpoints. Session-name subdirectories will be created here.
        tensorboard=Path(__file__).parent / "workspace/tensorboard",  # Tensorboard logs. Session-name subdirectories will be created here.
        tmcache=Path(__file__).parent / "workspace/tmodels_cache"     # Cache of pretrained target models, requires 22 GB disk space.
    )
    # paths = dict(
    ...
    args_parser.add_argument('--ftext', type=str, default="resnet101", choices=["resnet101", "resnet18", "resnext101_32x8d", "wide_resnet101_2", "densenet169"], help='Feature extractor')
